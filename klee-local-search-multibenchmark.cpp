...
int main(int argc, char* argv[]) {
  ...
  // Maps benchmark names to the coverage provided by the currently selected compiler flags.
  std::map<string, double> current_lcov_map;
  ...
  int iters=0;
  // Maximum number of flags to select.
  int iter_bound=10;
  
  // Line coverage to be improved.
  double best_multibenchmark_lcov_val=-1.0;
  ...
  while(iters<iter_bound) {
    ...
    */
    ...
    double best_flag_value=-1.0;

    cout<<"Starting value flag counting\n";
    for (std::map<string, double>::iterator it = flagValue.begin(); it != flagValue.end(); ++it)  {
      cout<<"Flag "<<(it->first)<<" has value "<<(it->second)<<"\n";
    ...
    // OSWALDO
    // Updating the current line coverage for the selected flags.
    for(j=0;j<num_benchmarks;++j) {
      string benchmark=benchmarks[j];
      map<string,data*> tempDataMap=benchmarkMap[benchmark];
      data *dataValue=tempDataMap[best_flag_name];
      current_lcov_map[benchmarks[j]]=dataValue->lcov;
     }

    

    cout<< "The best compiler flags after "<<iters<<" iterations are: "<<current_compiler_flags<< endl;
    cout<<"The coverage by benchmark is:"<<endl;
    for(j=0;j<num_benchmarks;++j) {
      string benchmark=benchmarks[j];
      cout<<benchmarks[j]<<": "<<current_lcov_map[benchmarks[j]]<<endl;
    }

      // See if any change happened compared to previous iteration
      if (best_multibenchmark_lcov_val - best_flag_value < delta)
	not_change_count++;
      else {
	not_change_count = 0;
	best_multibenchmark_lcov_val = best_flag_value;
       }

      // If it has not changed for two iterations, break
      if (not_change_count >= 2) {
	cout << "Multibenchmark coverage value has not changed for two iterations, stop\n";
	break;
      }
  ...
  for(i=0;i<num_benchmarks;++i) {
      ...
      cout<<benchmarks[i]<<": "<<current_lcov_map[benchmarks[i]]<<endl;
   }
